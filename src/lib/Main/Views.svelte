<script lang="ts">
	...
	$: sidebarEffective =
		...
		$dashboard?.hide_sidebar ||
		!$dashboard?.sidebar?.length
			? 0
	...
	$: navWidth =
		$layoutBreakpoint === 'narrow'
			? 'min(100%, calc(100vw - 4.25rem))'
			: `calc(100vw - (${sidebarEffective}px + 2rem + ${eyeWidth || 0}px + ${editViewButtonWidth || 0}px + 2rem + 0.1rem))`;
</script>
